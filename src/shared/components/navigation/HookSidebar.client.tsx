...
const routeHrefOf = (item: HookSidebarItem) => {
...
export function HookSidebar({
  ...
  const routed = items.some((item) => routeHrefOf(item));
  const routeIndex = items.findIndex((item) => routeHrefOf(item) === pathname);
    ...
    >
      {label && (
        <span
          ...
          className="pb-3 pl-0.5 pr-2 text-sm font-medium tracking-wide text-ink uppercase"
        >
          {label}
        </span>
      )}
      ...
      >
        ...
        {items.map((item, index) => {
          ...
          const rowProps = {
            ...
            className: cn(
              'rounded-lg py-1.5 pr-2 pl-5 text-left text-sm transition-colors duration-200 motion-reduce:transition-none',
          ...
          return href ? (
            <a
              ...
              ref={setRef}
              href={href}
              aria-current={isActive ? (href.startsWith('#') ? 'location' : 'page') : undefined}
            >
              {text}
            </a>
          ) : (
